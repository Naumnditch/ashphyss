...
import { CircularMotionLab } from '@/components/simulations/CircularMotionLab';
...
async function getSimContext() {
  try {
    const result = await query(
      `SELECT s.title, s.description, s.required_tier, c.id as chapter_id, c.chapter_number, c.title as chapter_title, t.id as topic_id, t.topic_name
       FROM simulations s
       JOIN chapters c ON c.id = s.chapter_id
       LEFT JOIN topics t ON t.id = s.topic_id
       WHERE s.url_path = '/simulations/circular-motion'
       LIMIT 1`
    );
...
export default async function CircularMotionSimulationPage() {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <div
            className="font-mono text-xs tracking-wide uppercase text-[#8f6428] mb-1.5"
          >
            {ctx ? `Chapter ${ctx.chapter_number}${ctx.topic_name ? ` · ${ctx.topic_name}` : ''}` : 'Simulation'}
          </div>
          <h1
            className="text-2xl sm:text-3xl font-semibold text-[#1b2a41]"
            style={{ fontFamily: 'Georgia, serif', letterSpacing: '-0.01em' }}
          >
            {ctx?.title || 'Circular Motion Lab: Horizontal, Vertical & Conical'}
          </h1>
          {ctx?.description && (
            <p
        ...
        {allowed ? <CircularMotionLab /> : <LockedContent requiredTier={requiredTier} title={ctx?.title || 'This simulation'} />}
      </div>
    </div>
  );
}
