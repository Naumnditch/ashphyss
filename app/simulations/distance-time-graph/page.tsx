  ...
  try {
    const result = await query(
      `SELECT s.title, s.description, s.required_tier, c.id as chapter_id, c.chapter_number, c.title as chapter_title, t.id as topic_id
       ...
       WHERE s.url_path = '/simulations/distance-time-graph'
       LIMIT 1`
    );
...
export default async function DistanceTimeGraphPage() {
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <div className="font-mono text-xs tracking-wide uppercase text-[#8f6428] mb-1.5">
            {ctx ? `Chapter ${ctx.chapter_number} · Describing Motion — Lesson 2.2` : 'Simulation'}
          </div>
          <h1
            className="text-2xl sm:text-3xl font-semibold text-[#1b2a41]"
            style={{ fontFamily: 'Georgia, serif', letterSpacing: '-0.01em' }}
          >
            {ctx?.title || 'Distance-Time Graph Explorer'}
          </h1>
          {ctx?.description && (
        ...
        {allowed ? <DistanceTimeGraphSimulator /> : <LockedContent requiredTier={requiredTier} title={ctx?.title || 'This simulation'} />}
      </div>
    </div>
  );
}
