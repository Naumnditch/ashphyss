...
import { CoulombLab } from '@/components/simulations/CoulombLab';
  ...
  try {
    const result = await query(
      `SELECT s.title, s.description, s.required_tier, c.id as chapter_id, c.chapter_number, c.title as chapter_title, t.id as topic_id, t.topic_name
       ...
       WHERE s.url_path = '/simulations/coulombs-law'
       LIMIT 1`
    );
...
export default async function CoulombsLawSimulationPage() {
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          >
            {ctx?.title || "Coulomb's Law Lab: Forces Between Charges"}
          </h1>
          {ctx?.description && (
            <p
        ...
        {allowed ? <CoulombLab /> : <LockedContent requiredTier={requiredTier} title={ctx?.title || 'This simulation'} />}
      </div>
    </div>
  );
}
