...
import { PendulumSimulator } from '@/components/simulations/PendulumSimulator';
  ...
  try {
    const result = await query(
      `SELECT s.title, s.description, s.required_tier, c.id as chapter_id, c.chapter_number, c.title as chapter_title, t.id as topic_id
       ...
       WHERE s.url_path = '/simulations/pendulum'
       LIMIT 1`
    );
...
export default async function PendulumSimulationPage() {
    ...
    >
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          ...
          >
            {ctx ? `Chapter ${ctx.chapter_number} · Making Measurements — Lesson 1.3` : 'Simulation'}
          </div>
          <h1
            className="text-2xl sm:text-3xl font-semibold text-[#1b2a41]"
            style={{ fontFamily: 'Georgia, serif', letterSpacing: '-0.01em' }}
          >
            {ctx?.title || 'Pendulum Lab: Measuring Time'}
          </h1>
          {ctx?.description && (
            <p
        ...
        {allowed ? <PendulumSimulator /> : <LockedContent requiredTier={requiredTier} title={ctx?.title || 'This simulation'} />}
      </div>
    </div>
  );
}
