...
import { HalfLifeSimulator } from '@/components/simulations/HalfLifeSimulator';
  ...
  try {
    const result = await query(
      `SELECT s.title, s.description, s.required_tier, c.id as chapter_id, c.chapter_number, c.title as chapter_title, t.id as topic_id
       ...
       WHERE s.url_path = '/simulations/half-life'
       LIMIT 1`
    );
...
export default async function HalfLifeSimulationPage() {
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          <div className="font-mono text-xs tracking-wide uppercase text-[#8f6428] mb-1.5">
            {ctx ? `Chapter ${ctx.chapter_number} · Radioactivity — Lessons 23.2 & 23.3` : 'Simulation'}
          </div>
          <h1
            className="text-2xl sm:text-3xl font-semibold text-[#1b2a41]"
            style={{ fontFamily: 'Georgia, serif', letterSpacing: '-0.01em' }}
          >
            {ctx?.title || 'Half-Life Lab: Random Decay, Predictable Law'}
          </h1>
          {ctx?.description && (
            <p
        ...
        {allowed ? <HalfLifeSimulator /> : <LockedContent requiredTier={requiredTier} title={ctx?.title || 'This simulation'} />}
      </div>
    </div>
  );
}
